...
public:
    ...
    void AddToCache(const uint8_t *entry)
    {
        if (this->done) {
         ...
         bucket_index = Util::ExtractNum(entry, entry_size_, begin_bits_, log_num_buckets_);
         buckets_[bucket_index].file.Write(buckets_[bucket_index].write_pointer, entry, entry_size_);
         buckets_[bucket_index].write_pointer += entry_size_;
    }
    ...
    {   
        if(th1.joinable())this->th1.join(); 
        for (auto& b : buckets_) {
            b.file.FreeMemory();
            // the underlying file will be re-opened again on-demand
            b.underlying_file.Close();
        }
        this->prev_bucket_buf_.reset();
        this->memory_start_.reset();
        this->memory_start_2.reset();//зЂвт
    ...
    uint8_t *ReadEntry3(uint64_t position)
    {
...
private:
    ...
    void newsb()
    {
        const uint8_t nowbucket = this->next_bucket_to_sort;
        if (nowbucket == 0)
        {
            if (th1.joinable())this->th1.join(); 
            SortBucket();
            this->th1 = std::thread([=](uint32_t acc) { this->SortBucketThread(acc); }, this->next_bucket_to_sort);
        }
        else if (nowbucket < buckets_.size() - 1)
        {
            if (th1.joinable())this->th1.join(); 
            this->done = true;
            this->final_position_start = this->final_position_end;
            this->final_position_end += this->Threadpointer;
            this->next_bucket_to_sort += 1;
            std::swap(memory_start_, memory_start_2);
            this->th1 = std::thread([=](uint32_t acc) { this->SortBucketThread(acc); }, this->next_bucket_to_sort);
        }
        else
        {
    ...
    std::unique_ptr<uint8_t[]> memory_start_2;
    
    uint64_t Threadpointer;


    void SortBucketThread(uint64_t const bucket_i)
    {
        if (!memory_start_2) {
            // we allocate the memory to sort the bucket in lazily. It'se freed
            // in FreeMemory() or the destructor
            memory_start_2.reset(new uint8_t[memory_size_]);
        }
        ...
        if (!force_quicksort &&
            ...
            std::cout << "\tBucket " << bucket_i << " QS. Ram: " << std::fixed
                ...
                << "GiB, qs min: " << qs_ram << "GiB. force_qs: " << force_quicksort
                << std::endl;
            b.underlying_file.Read(0, memory_start_2.get(), bucket_entries * entry_size_);
            QuickSort::Sort(memory_start_2.get(), entry_size_, bucket_entries, begin_bits_ + log_num_buckets_);
        }
        // Deletes the bucket file
        std::string filename = b.file.GetFileName();
        b.underlying_file.Close();
        ...
        this->Threadpointer = b.write_pointer;
    }
